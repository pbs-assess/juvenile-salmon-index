...
Now we prep the SPDE mesh necessary to account for spatial heterogeneity and fit Sean's models. To account for those seasonal effects we include z-scored year day as a covariate. To account for differences in tow length (the average high seas tow was substantially longer), I fit the models to cpue (i.e. catch / tow duration). The data are not log transformed prior because the tweedie distribution is intended for zero bounded, positive data and can account for zero-inflation (similar to a gamma hurdle model but with more flexibility). Note that the run time for these models can be pretty long so I've included the RDS files that you can simply load once you move them to the appropriate location.
...
m1_tw <- readRDS(here::here("data", "modelFits", "ck1_yrInt_tw.rds"))
m2_tw <- readRDS(here::here("data", "modelFits", "ck2_yrInt_tw.rds"))

# alternatively run models
...
pred_m1 <- predict(m1_tw, newdata = pred_grid1, return_tmb_object = TRUE)
pred_m2 <- predict(m2_tw, newdata = pred_grid2, return_tmb_object = TRUE)
...
2. Generally the uncertainty is similar, however little survey effort and low catches (i.e. 0) within the IPES grid in years like 2014 means that estimates there are probably biased low.  
