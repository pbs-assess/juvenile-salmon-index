...
As mentioned above, even though the model is parameterized using a larger spatial domain, we can constrain predictions to focus on current IPES survey grid. 

```{r spatial_preds, echo = TRUE}
# import utm grid to generate predictions on
exp_grid <- readRDS(here::here("data", "exp_pred_ipes_grid_utm.rds")) %>% 
  filter(!year %in% c("1995", "2022"),
         !is.na(dist_to_coast_km))

# generate predictions
preds <- predict(fit_st_survey, exp_grid)
preds_hss <- preds %>% filter(season_f == "su", survey_f == "hss")
preds_ipes <- preds %>% filter(season_f == "su", survey_f == "ipes")
