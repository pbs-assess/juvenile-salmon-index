...
fit_sims_list <- furrr::future_map(
  ...
  function(x) {
    ...
    sdmTMB(
      ...
      silent = FALSE#,
      # priors = sdmTMBpriors(
      #   b = normal(c(0, 0, 0, 2, 2, 0, 0, 0, 0, 0), 
...
fix <- tidy(fit_nb2_yday3, effects = "fixed")
ran <- tidy(fit_nb2_yday3, effects = "ran_pars")

# pull upsilon estimate separately (not currently generated by predict)
est <- as.list(fit_nb2_yday3$sd_report, "Estimate", report = TRUE)
se <- as.list(fit_nb2_yday3$sd_report, "Std. Error", report = TRUE)
upsilon <- data.frame(
  term = "sigma_U",
  log_est = est$log_sigma_U,
  log_se = se$log_sigma_U
...
true_pred <- predict(fit_nb2_yday3,
                     newdata = index_grid_hss %>%
                       filter(season_f == "wi"),
                     se_fit = FALSE, re_form = NULL, return_tmb_object = TRUE)
