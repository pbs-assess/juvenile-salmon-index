...
# add prior on survey effects to constrain 
...
fits_list_nb2_prior <- furrr::future_map2(
...
dat_tbl$fit <- fits_list_nb2_prior
saveRDS(dat_tbl, here::here("data", "fits", "all_spatial_varying_nb2_prior.rds"))

dat_tbl <- readRDS(here::here("data", "fits", "all_spatial_varying_nb2_prior.rds"))
...
saveRDS(sims_list,
        here::here("data", "fits", "nb_mcmc_draws_nb2_prior.rds"))


dat_tbl$sims <- sims_list 
...
for (i in seq_along(sp_vec)) {
  ...
  saveRDS(
    ...
    here::here("data", "fits", "sim_fit", 
               paste(sp_vec[i], "_nb2_prior.rds", sep = ""))
  )
}
...
sim_fit_list <- purrr::map(
  sp_vec,
  ~ readRDS(
    here::here("data", "fits", "sim_fit", paste(.x, "_nb2_prior.rds", sep = ""))
  )
)
...
saveRDS(
  index_list,
  here::here("data", "fits", "sim_fit", "nb2_prior_index.rds")
)
...
for (i in seq_along(sp_vec)) {
  ...
  saveRDS(
    sim_ind_list,
    here::here("data", "fits", "sim_fit",
               paste(sp_vec[i], "_sim_index_prior.rds", sep = ""))
  )
}
